...
int BootProgram(bool forANNIndexTestTool, std::map<std::string, std::map<std::string, std::string>> *config_map,
                const char *configurationPath, VectorValueType valueType, DistCalcMethod distCalcMethod,
...
#undef DefineVectorValueType
    ...
    if (opts->m_generateTruth)//搜索时跳过了
    {
        ...
        std::shared_ptr<Helper::ReaderOptions> queryOptions(
            new Helper::ReaderOptions(opts->m_valueType, opts->m_dim, opts->m_queryType, opts->m_queryDelimiter));
...
#define DefineVectorValueType(Name, Type)                                                                              \
    ...
    {                                                                                                                  \
        COMMON::TruthSet::GenerateTruth<Type>(querySet, vectorSet, opts->m_truthPath, distCalcMethod,                  \
                                              opts->m_resultNum, opts->m_truthType, index->m_pQuantizer);              \
    }
